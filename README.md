...
**Re-run the command above after upgrading remo.** The activation script is a
static snapshot — upgrading remo does not rewrite a file you already generated,
and a stale script can break completion in ways remo cannot report (your shell
parses the file itself, so remo is not running when it fails). Each generated
script carries a `generated by remo <version>` header so you can check.
