...
## Recent Changes
- 024-discovery-resilience: Made the web console tolerant of intermittent instance connectivity — a transient discovery failure is now a *degraded* state, not a *destroyed* one. Previously one timed-out discovery cycle unconditionally replaced an `ok` snapshot holding N session targets with a `timeout` snapshot holding zero (`web/discovery.py` `_run_and_store`), the OK-only target index dropped the host from `GET /sessions` *and* from terminal open/reattach authorization (`web/api/terminals.py`), the workspace filtered the unresolvable panes out and unmounted their live terminals, and the rail collapsed the host into a red error block — while a page refresh restored everything, proving the instance was fine. Four coordinated parts: **(1) backend retention** — on a retryable failure (`timeout`/`unreachable`) with a prior `OK` snapshot, `DiscoveryService` stores a retained snapshot (prior capability/targets/region, `status=ok`, `stale=true`, `consecutive_failures`+1, latest failure in `error`) until a monotonic-clock grace budget (`REMO_WEB_DISCOVERY_OFFLINE_GRACE_S`, default 120s, 0 disables) measured since the last success expires; keeping `status=ok` is what keeps targets in `_targets_by_id`, so terminal open/WS-reattach authorization needs zero changes. Non-retryable statuses (`auth_failed`/`no_remo_host`/`incompatible_protocol`/`malformed`) surface immediately — retention would hide a real fix-me. Staleness rides as additive fields on `DiscoverySnapshot`/`InstanceOut` (`stale`, `last_ok_at`, `consecutive_failures`), NOT a new `InstanceStatus` member (wire-breaking, and `providerMeta.ts`'s exhaustive `Record` would gate it). **(2) timeout budget** — `_discover_one_sync` runs two sequential calls each given the full `discovery_timeout_s` while the outer `asyncio.wait_for` bounded both at that same value, so a slow capabilities call starved the sessions call into a spurious timeout; the outer bound is now 2x + slack. **(3) rail** — `railModel.ts` grows `isStale` beside (never replacing) `isError`: rows stay, the group dims with a compact "not responding · retrying" chip; grace-exhausted and deterministic failures render today's error block byte-identically. **(4) workspace** — `WorkspacePane` resolves attached panes against an in-memory sticky last-known-target map (refreshed on live resolve, dropped on close, never persisted), so a pane is never unmounted because discovery blinked — even past the grace window; the card's own `connectionState` (whose WS auto-reconnect exhausts in ~5.5s, well inside one 15s poll) is the liveness truth. `InstanceOut` change regenerated all four artifacts (Principle IV). No registry schema change; no new runtime deps.
