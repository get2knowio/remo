...
class TestCompletionCommand:
    ...
    def test_completion_script_carries_version_stamp(self, shell, target):
        ...
        stale file identifiable without counting line numbers.
        """
        result = _invoke("completion", shell)
        assert result.exit_code == 0
        assert f"generated by remo {remo_cli.__version__}" in result.output
        assert f"remo completion {shell} {target}" in result.output
