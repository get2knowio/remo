...
class TestCompletionCommand:
    ...
    def test_install_help_does_not_claim_shell_env_detection(self):
        """#115 changed detection to prefer the process tree, but the help text
        kept saying "detected from $SHELL" — and shipped that way in 3.0.0.
        The docstring restates behavior defined elsewhere, so nothing but a
        test keeps the two in step (Principle VIII).
        """
        result = _invoke("completion", "install", "--help")
        assert result.exit_code == 0
        assert "detected from $SHELL" not in result.output
